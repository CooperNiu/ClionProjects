...
{ /* 将线性链表L重置为空表，并释放原链表的结点空间 */
    ...
    {
        p=q=(*L).head->next;//从首元结点开始
        (*L).head->next=NULL;//线性链表的头结点的指针域赋空
        while(p!=(*L).tail)//遍历整个链表
        {
            p=q->next;//p存储即将释放的结点指针
            free(q);//释放
            q=p;//传承是为了更好的继续
        }
        free(q);//任务完成,功成身退
        (*L).tail=(*L).head;//就像是初始化一样,让头指针和尾指针相等
        (*L).len=0;//长度重置为0
    }
    return OK;
}
...
{ /* 将指针s(s->data为第一个数据元素)所指(彼此以指针相链,以NULL结尾)的 */
    ...
    /* 的尾结点 */
    int i=1;//计数器
    (*L).tail->next=s;//将s所指的一串结点接入
    while(s->next)//遍历,取尾节点和长度
    {
        s=s->next;
        i++;
    }
